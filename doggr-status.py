...
class check_status:
    ...
    def calc_status(self):
        ...
        try:
            status["last_prod"] = max(status.keys(), key=(lambda k: status[k]))
        ...
        db.doggr.update_one(
            {"api": self.api}, {"$set": {"status_calc": status}}, upsert=False
        )
...
df_ = pd.DataFrame(list(db.doggr.find({"oil_cum": {"$gt": 0}}, {"api": 1})))
apis = df_["api"].values
